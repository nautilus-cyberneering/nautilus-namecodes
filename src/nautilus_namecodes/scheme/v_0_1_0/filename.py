...
class Gold(Way):
    ...
    def get_codes_r(self, all_codes: AllCodes) -> List[Tuple[int, str]]:
        ...
        if isinstance(self.gold_alternative_or_base, GoldAlternative):
            return [
                *self.gold_alternative_or_base.get_codes_r(all_codes),
                *self.get_codes(all_codes),
            ]
        if isinstance(self.gold_alternative_or_base, Base):
            return [
                *self.gold_alternative_or_base.get_codes_r(all_codes),
                *self.get_codes(all_codes),
            ]
